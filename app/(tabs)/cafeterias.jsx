import { ScrollView, Text, View, Linking, StyleSheet, Pressable } from "react-native";
...
const OpenHoursTable = [
  ...
  {day: 'Niedziela', hours: "9:00 - 18:00"},
]; 

const OpeningHours = ( props )=>{
  return(
    <Animated.View entering={FadeIn.duration(300)} exiting={FadeOut.duration(1)} style={style.timeTable}>
      <Text style={globalStyles.bodyText}>{props.day}</Text>
      <Text style={globalStyles.bodyText}>{props.hours}</Text>
    </Animated.View>
  )
};
...
const TimeTable = ( {active} ) => {
  ...
  const OpenHours = OpenHoursTable.map( (opens, index) => { return <OpeningHours key={index} day={opens.day} hours={opens.hours} /> });
...
return (
  <View style={[style.cafeteriaContainer, globalStyles.boxShadow]}>
    <View>
      <Text style={globalStyles.h5}>{props.caffeteriaName}</Text>
      <Text style={globalStyles.bodyText}>{props.caffeteriaAddress}</Text>
        <View style={style.buttonContainer}>
          // poprawa ButtonWithIcon bo się rozjezdzaja icony
          <ButtonWithIcon style={[globalStyles.fullButton, globalStyles.buttonFlex]} buttonText={'Kierunek'} onPress={() => Linking.openURL(props.localization)} color="#fffcf2" path="M541.9 139.5C546.4 127.7 543.6 114.3 534.7 105.4C525.8 96.5 512.4 93.6 500.6 98.2L84.6 258.2C71.9 263 63.7 275.2 64 288.7C64.3 302.2 73.1 314.1 85.9 318.3L262.7 377.2L321.6 554C325.9 566.8 337.7 575.6 351.2 575.9C364.7 576.2 376.9 568 381.8 555.4L541.8 139.4z" variant="full"/>
          <ButtonWithIcon style={[globalStyles.emptyButton, globalStyles.buttonFlex]} buttonText={'Zadzwoń'} onPress={() => Linking.openURL(`tel:${props.phoneNumber}`)} color="#31572c" path="M224.2 89C216.3 70.1 195.7 60.1 176.1 65.4L170.6 66.9C106 84.5 50.8 147.1 66.9 223.3C104 398.3 241.7 536 416.7 573.1C493 589.3 555.5 534 573.1 469.4L574.6 463.9C580 444.2 569.9 423.6 551.1 415.8L453.8 375.3C437.3 368.4 418.2 373.2 406.8 387.1L368.2 434.3C297.9 399.4 241.3 341 208.8 269.3L253 233.3C266.9 222 271.6 202.9 264.8 186.3L224.2 89z" variant="empty"/>
        </View>
    </View>
    <View >
      <TimeTable active={active}/>
      <View style={{display:'flex', alignItems:'center', marginTop:24}}>
        <Animated.View style={{transitionProperty:'all', transitionTimingFunction: 'ease-in', transitionDuration: 200, transform:[{rotate: active ? '180deg' : '0deg'}]}}>
